...
class source:
		...
		try:
			...
			for post in posts:
				try:
					...
					# self.prototype_url.append([links,t])
					
				except:
					pass
			# print ("SCENEDOWN PASSED", self.prototype_url)		
        ...
        try:
			...
			for post in posts:
				try:
					...
					pass
			print "%s TV QUERY PASSED %s" % (self.base_link, self.prototype_url)
			return self.prototype_url
        except:
            return			
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
			sources = []
			for title,url in self.prototype_url:
				quality = "SD"
